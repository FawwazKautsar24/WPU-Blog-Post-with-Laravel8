...
@section('container')
    ...
    @if ($posts->count())
        <div class="card mb-3">
            <!-- @php
                $url = "https://api.unsplash.com/search/photos/?client_id=rv2lzLf_StpP7NanUooLnSC5RyD-C1QLOo8jmelArsw&&per_page=1&&query={{ $posts[0]->category->name }}";
                $url_img = $url;
            @endphp -->
            @if ($posts[0]->image)
                    ...
                    <div class="col-md-4 mb-3">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <p>By. <a href="/posts?author={{ $post->author->username }}" class="text-decoration-none">{{ $post->author->name }}</a> {{ $post->created_at->diffForHumans() }}</a></p>
